<script lang="ts">
	...
	import { LayoutDashboard, Map, CalendarDays, Users, Settings, Menu, X } from 'lucide-svelte';
				...
				<div class="p-6">
					<a href="/" class="flex items-center gap-3 group">
						...
						<div class="flex flex-col">
							...
							<span class="text-sm text-gray-500 mt-1"> Hlaks Tours Dashboard </span>
						</div>
					</a>
				</div>
			</div>
				...
				{#each navigationItems as item (item.href)}
					<a
						...
						class="flex items-center px-4 py-3 text-gray-600 rounded-lg hover:bg-emerald-50 hover:text-emerald-600
						   {$page.url.pathname.match(item.match) ? 'bg-emerald-50 text-emerald-600' : ''}"
					>
